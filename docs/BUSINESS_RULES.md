...
- **Timezone & day boundaries.** Every user owns an authoritative IANA `User.TimeZoneId`, set via `PUT /api/me/timezone` and minted into the access-token `tz` claim at login and on each refresh (claim mechanics in [AUTHENTICATION.md](AUTHENTICATION.md)). Handlers resolve "today"/day/week boundaries from `ICurrentUser.TimeZoneId` (UTC fallback) through `LocalDayResolver` — **never from a client-sent parameter**. Per-session `WorkoutSession.ClientTimezone` records where a session was logged; coach reads and the coach session-list date filter resolve in the **trainee's** zone (so a coach abroad sees the trainee's local days, e.g. a trainer in Vietnam reviewing a trainee in Canada). Both clients report their device IANA zone on login (Angular via `Intl`, Flutter via `flutter_timezone`) and render another trainee's data in that trainee's captured zone (Flutter via the `timezone` db). No stored instant is ever rewritten — only the derived local day/week.

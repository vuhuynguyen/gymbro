...
# Part C — Architecture (deferred re-architectures)

## C1. Persistence dependency inversion

**Problem (audit HIGH finding).** `BuildingBlocks.Infrastructure.Persistence` — nominally the bottom-of-stack
shared kernel — project-references all six feature modules and reaches into every one's `.Entities` namespace,
because it owns every entity's `IEntityTypeConfiguration`, the `AppDbContext` typed `DbSet`s, and all 15
repositories. The documented "modules → kernel" dependency is therefore inverted.

**Why it is partly inherent (not pure tech-debt).** Of the 27 EF configs, **4 are cross-module FK configs** that
wire foreign keys spanning module boundaries and so reference *two* modules' entities:
`PerformedExerciseConfiguration` and `PlanWorkoutExerciseConfiguration` → `Exercise`; `LoggedItemConfiguration` and
`PlanMealItemConfiguration` → `Food`. These cannot live inside either module without it referencing the other's
`.Entities` (forbidden by `ModuleBoundaryConventionTests`); a place that legitimately sees both modules is
required. Combined with one `AppDbContext` owning all `DbSet`s, **the kernel must reference the feature modules** —
a standard modular-monolith trade-off. Merely moving configs into modules is cosmetic: the `DbSet`s + cross-module
configs keep the dependency direction unchanged. A genuine inversion is a **re-architecture**, not a refactor.

### Option B (recommended) — module-contributed model, kernel made module-free
Keep the single `AppDbContext` + single migration chain (lowest disruption), but make the *kernel* stop
referencing modules; the *composition root* (`Presentations/WebApi`), which already references everything, wires
the model.
1. **New module-referenceable infra lib** (e.g. `…Persistence.Abstractions`) holding the generic `Repository<T>`
   base (retargeted concrete `AppDbContext` → `DbContext`), `IDbContextServices`/interceptors, and an
   `IModelConfiguration` seam. Modules reference *this*, never the concrete-context project.
2. **`AppDbContext` drops typed `DbSet`s** (use `Set<T>()`) and builds its model from an injected set of config
   assemblies/sources instead of `ApplyConfigurationsFromAssembly(typeof(AppDbContext).Assembly)`. No module
   entity type is referenced.
3. **Single-module configs (22) move into their owning module** (`Modules.X/Infrastructure/Persistence/`); each
   module exposes its config assembly as a source.
4. **Cross-module FK configs (4) move to the composition root** (or a dedicated `…Persistence.Relationships`
   assembly referenced only by WebApi) — registered as an additional config source.
5. **Repositories (15) move into their modules**, depending on the `DbContext` base via the relocated
   `Repository<T>`; per-module `AddXModulePersistence()` extensions register them (replacing the god-registration
   in `PersistenceExtensions`).
6. **Drop the kernel's `ProjectReference`s to the modules**; widen `ModuleBoundaryConventionTests` to scan the
   kernel too (remove the documented exemption).

Net: kernel references no modules; each module owns its entities + configs + repos; one DbContext + one migration
chain; DB-level cross-module FKs preserved (configured at the composition root); outbox/audit/tenant-filter
interceptors stay on the base context in the kernel.

### Option A (heavier) — per-module DbContexts
Each module gets its own `DbContext` + migration chain; cross-module FKs are **dropped at the DB level** and
enforced only in the application layer (the existing `ValidateExerciseIdsQuery` / `ValidateFoodIdsQuery` MediatR
checks already do this). True bounded-context persistence, but ~7 migration chains, harder cross-module
transactions (extend the cross-store-transaction pattern), and loss of DB-enforced referential integrity for the
four cross-module FKs. Only worth it if a module is ever split into its own service.

### Sequencing & validation (dedicated effort, Docker up)
Each step must end green: `dotnet build` + unit tests + **`dotnet ef migrations has-pending-model-changes` →
"No changes"** (proves the relocation didn't alter the EF model) + the full **integration suite** (boots the app,
applies migrations to real Postgres — the real safety net). Do NOT attempt without the integration suite. Stage 1
(configs) → Stage 2 (repos + per-module DI) → Stage 3 (drop `DbSet`s + config-source injection + drop project
refs); each independently green and committable.

**Status.** Not started. Safe interim in place: `ModuleBoundaryConventionTests` documents the kernel→module
exemption so the green test no longer overstates the boundary guarantee.

## C2. Timezone consistency — remaining work

**As-built (shipped).** Day/week bucketing now anchors on the trainee's captured zone, not UTC: a single
`LocalDayResolver.LocalDateOf(instant, ianaZone)` helper (UTC fallback for absent/legacy zones) drives
`SessionMapping.ComputePlanWeek` and `GetMyProgressHandler.WeekStart` — each session buckets in its own
`ClientTimezone` — and `GetMyNutritionTodayHandler`'s "today" fallback derives from the request's zone instead of
`UtcNow`. The nutrition daily-log flow was already trainee-local (the client sends the local date). A cross-zone
unit test pins it (`SessionMappingTests.Plan_week_buckets_on_the_trainees_local_date_not_utc`).

**Remaining (authoritative anchor + edges + clients).** The per-session `ClientTimezone` is the immediate anchor,
but the two clients write it in incompatible formats (Angular = IANA `Asia/Bangkok`; Flutter = abbreviation
`+07`) and there is no per-user zone. To finish:
1. **`User.TimeZoneId` (IANA) — the authoritative anchor:** entity field + `SetTimeZone` (validate via
   `TimeZoneInfo.TryFindSystemTimeZoneById`) + EF config (`AppDbContext` migration) + expose on the profile DTO +
   a set-timezone endpoint; populate from the client's IANA id on register/profile-sync. Use it as the fallback
   when a session's `ClientTimezone` is null/unparseable.
2. **History date filters** (`GetMyWorkoutHistoryHandler`, `ListSessionsHandler`): convert the from/to local dates
   to a UTC instant range using the trainee's zone instead of `DateTimeKind.Utc` (off-by-one at both ends today).
3. **Nutrition metric fallbacks** (`LogMetricEntryHandler`, `GetMyNutritionMetricsHandler`): fall back to the
   user's zone, not `UtcNow`.
4. **Clients:** Flutter sends an IANA id (add `flutter_timezone`) for session-start AND the nutrition `timezone`
   param (currently null), matching Angular; both clients render ANOTHER trainee's data in the TRAINEE's zone
   (coach view), not the viewer's `.toLocal()` / `DateTime.now()`. Optional one-off backfill of `User.TimeZoneId`
   from the most-recent IANA `ClientTimezone`. No instant data is rewritten — only derived day/week.

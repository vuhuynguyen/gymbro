...
/// Only PLANNED days (<c>Source == FromAssignment</c>) count: an ad-hoc self-logged day carries no plan to
/// adhere to (its adherence is 100% by convention) and would inflate the trend, so it is excluded. Per-day
    ...
    : IRequestHandler<GetMyNutritionAdherenceQuery, Result<NutritionAdherenceDto>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Self-scoped, cross-gym base query (tenant filter bypassed, soft-delete re-applied), narrowed to
        // PLANNED days only — an ad-hoc day has no plan to adhere to.
        var planned = logRepository.QueryOwnAcrossGyms(currentUser.UserId)
            .AsNoTracking()
            .Where(l => l.Source == NutritionSource.FromAssignment);
        ...
        if (!hasPlan)
            return Result<NutritionAdherenceDto>.Success(
                new NutritionAdherenceDto(HasPlan: false, Days: [], CurrentWeekAvgPct: null));
        ...
        return Result<NutritionAdherenceDto>.Success(
            new NutritionAdherenceDto(HasPlan: true, days, currentWeekAvgPct));
    }
}
